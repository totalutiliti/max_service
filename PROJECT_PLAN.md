...
- O CI executa lint, builds, testes funcionais, auditoria de dependências e scanner de segredos; em ambiente Docker limpo, também escaneia as imagens com Trivy fixado por digest, aplica todas as migrations e prova RLS e proteção contra conflitos de agenda.
- O executor de migrations serializa réplicas por advisory lock, exige sequência contínua e registra SHA-256 normalizado; o pipeline recria o esquema em banco descartável, comprova repetição idempotente e recusa drift de qualquer arquivo já aplicado.
...
- O Docker coleta essas séries com Prometheus 3.13.1 LTS em build reproduzível, com origem, bases e interface fixadas e gRPC corrigido, retenção local limitada, SLI separado de probes, seis recording rules, sete alertas testados e runbook; serviço gerenciado, Alertmanager e plantão continuam gates externos.
