...
- Criação de pedido, envio de proposta, aceite, mensagens de texto, comandos JSON do atendimento, agenda e ciclo do serviço usam `Idempotency-Key` vinculada à assinatura BFF→API; chave, efeito e resposta são confirmados na mesma transação, com replay observável e bloqueio de reutilização com outro conteúdo.
...
- Smoke tests exercitam liveness, readiness, cockpit operacional, bloqueio entre perfis, rejeição de cabeçalhos internos não assinados e pares concorrentes de vinte ações idempotentes cobertas.
