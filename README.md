...
- Landing page e primeira experiência SaaS navegável para cliente, profissional, parceiro, anunciante e administração.
...
- A Operação agenda entregas semanais ou mensais desses relatórios para destinatários sintéticos consentidos, pode pausar ou reativar a recorrência e simular cada ciclo; a fotografia preserva apenas agregados, checksum, versão e trilha append-only, enquanto o provedor externo permanece tecnicamente bloqueado.
...
- O quinto perfil, **Anunciante**, envia peças com destino HTTPS e público opcional por categoria/região; nenhuma campanha entra no ar sem decisão humana e justificativa da Operação.
- O cliente recebe no pedido somente publicidade contextual identificada como **Patrocinado**, com explicação do motivo da exibição; impressões e cliques são agregados sem perfil comportamental ou identidade bruta do visitante.
...
- Os cinco perfis têm uma central persistente de notificações, com atualização automática do contador, leitura individual ou em massa.
...
- Cliente, profissional, parceiro e anunciante possuem uma Central de Privacidade na Conta: registram acesso, correção, restrição, revogação ou exclusão e podem acompanhar todas as decisões. Pedidos de acesso geram um JSON minimizado, sem conteúdo de terceiros, com recibo e checksum.
...
- Depois da resolução, o parceiro pode abrir uma única contestação formal; a Operação assume a revisão e registra decisão acolhida ou mantida com justificativa, enquanto ambos acompanham a mesma trilha imutável.
...
- Smoke tests exercitam liveness, readiness, cockpit operacional, bloqueio entre perfis, rejeição de cabeçalhos internos não assinados e comandos concorrentes idempotentes, incluindo uploads privados e moderação publicitária.
...
- [Publicidade contextual moderada](docs/security/contextual-advertising.md)
- [Entregas consentidas de relatórios](docs/security/report-delivery.md)
...
- [Reconciliação do cofre privado](docs/operations/private-storage-reconciliation.md)
...
Esta entrega é uma fundação local, ainda com identidades e dados fictícios. As sessões demonstrativas exercitam expiração, revogação, cookie seguro e autorização, mas não substituem cadastro público, senha forte, confirmação de contato, recuperação de conta ou MFA administrativo. Documentos, fotos de pedidos, imagens das conversas e anexos de atendimento aceitam somente arquivos sintéticos; antivírus, quarentena automatizada, criptografia gerenciada e política final de retenção continuam obrigatórios antes de dados reais. A Central de Privacidade materializa solicitações, decisões e exportação estruturada, mas não executa exclusão física ou anonimização automática antes da aprovação jurídica da retenção. Mensagens e notificações usam sincronização incremental automática, e o canal Web Push opt-in já opera com chaves VAPID exclusivamente locais, preferências por assunto e horário silencioso; WebSocket, e-mail/SMS, alertas externos, entrega externa dos relatórios e escalonamento automático de SLA, pagamentos reais e integrações externas permanecem desativados. A captura pública de indicação registra somente interesse e consentimento: não cria conta, não confirma contato e não dispara comunicação externa. A análise preventiva dessas indicações usa somente padrões internos explicáveis, não consulta crédito ou antecedentes e exige decisão humana quando encontra sinal. A aprovação operacional coloca o interessado em `approved`, aguardando onboarding; somente uma conversão posterior com conta de prestador pode torná-lo `active`. Conversas, notificações internas, assinaturas push, preferências versionadas de entrega, solicitações e recibos de privacidade, agendamentos e fotografias agregadas de relatórios, atendimentos do parceiro, catálogo operacional, regiões, bairros, coberturas profissionais, campanhas e reservas de cupons, campanhas publicitárias moderadas, impressões e cliques agregados, rede do parceiro, ciclo do agendamento, cancelamentos, tratamento de chamados, estados da verificação, anexos privados, versões documentais, avaliações e ledger financeiro sandbox são persistentes. As credenciais fixas do `compose.yaml` existem somente para desenvolvimento local; o par VAPID fica no `.env` ignorado pelo Git, e nenhum desses valores pode ser reutilizado em produção.
