...
test("limita a retenção local a mil amostras", () => {
  ...
  assert.equal(metrics.applicationRequests.success, 1_010);
  assert.equal(metrics.applicationRequests.error, 0);
  const applicationDuration = metrics.durationSeries.find(
    (series) => series.traffic === "application",
  );
  assert.equal(applicationDuration?.count, 1_010);
  assert.equal(applicationDuration?.buckets.at(-1)?.count, 1_010);
});
