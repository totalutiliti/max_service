...
export const maximumProviderDocumentBytes = 2_097_152;

export function validateProviderDocumentFile(originalName: string, contentType: string, bytes: Buffer) {
  if (bytes.length < 4 || bytes.length > maximumProviderDocumentBytes) {
  ...
  const valid = contentType === "application/pdf"
    ...
    : contentType === "image/jpeg"
      ...
      : contentType === "image/png"
        ...
        : false;
  if (!valid) throw new BadRequestException("Tipo, extensão ou assinatura do arquivo não permitidos.");
  return name;
}
