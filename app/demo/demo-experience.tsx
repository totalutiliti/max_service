  ...
  return (
    <>
      ...
      <section className="dashboard-hero">
        <div><span className="small-label">NOVO PEDIDO</span><h2>Precisa de ajuda em casa?</h2><p>Conte o que precisa e receba propostas de profissionais disponíveis na sua região.</p><button className="button" data-testid="new-service-request" onClick={() => openRequest()}>Pedir um serviço →</button></div>
...
function ProviderView({ notify }: { notify: (message: string) => void }) {
  ...
  const normalizedOpportunityQuery = opportunityQuery.trim().toLocaleLowerCase("pt-BR");
  const filteredOpportunities = opportunities.filter((request) => !normalizedOpportunityQuery
    || [request.publicCode, request.title, request.categoryName, request.neighborhood, request.city]
      .some((value) => value.toLocaleLowerCase("pt-BR").includes(normalizedOpportunityQuery)));
  ...
  return (
    <>
      ...
      <div className="dashboard-columns wide-left">
        <section className="dashboard-section"><div className="dashboard-section-title"><div><small>OPORTUNIDADES PRÓXIMAS</small><h2>Pedidos disponíveis</h2></div><button onClick={() => { setLoading(true); setRefresh((value) => value + 1); }}>Atualizar ↻</button></div><div className="records-toolbar"><label><span>Buscar oportunidade</span><input data-testid="opportunity-search" value={opportunityQuery} onChange={(event) => setOpportunityQuery(event.target.value)} placeholder="Código, serviço, categoria ou região" /></label><span className="records-counter">{filteredOpportunities.length} pedido(s)</span></div><div className="opportunity-list">{loading && <div className="data-state">Buscando oportunidades...</div>}{!loading && opportunities.length === 0 && <div className="data-state"><strong>Nenhum pedido disponível agora.</strong><span>Novos pedidos aparecerão aqui automaticamente.</span></div>}{!loading && opportunities.length > 0 && filteredOpportunities.length === 0 && <div className="data-state"><strong>Nenhuma oportunidade encontrada.</strong><span>Ajuste a busca para consultar outros pedidos.</span></div>}{filteredOpportunities.slice(0, 5).map((request) => <Opportunity key={request.id} request={request} onSelect={() => setSelected(request)} />)}</div></section>
...
function ProposalDialog({ request, onClose, onSaved, notify }: { request: PersistedRequest; onClose: () => void; onSaved: () => void; notify: (message: string) => void }) {
  ...
  return (
    <div className="dialog-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
      <section className="request-dialog proposal-dialog" data-testid="proposal-dialog" role="dialog" aria-modal="true" aria-labelledby="proposal-title">
        <button className="dialog-close" ref={closeRef} onClick={onClose} aria-label="Fechar">×</button>
...
function PartnerView({ notify }: { notify: (message: string) => void }) {
  ...
  return (
    <>
      <DashboardHeader role="parceiro" eyebrow="Área do parceiro" title="Sua rede, com origem e status claros."><button className="button button-small" data-testid="new-referral" onClick={() => setInviteOpen(true)} disabled={!data || data.categories.length === 0}>Nova indicação</button></DashboardHeader>
...
function OperationReferralDialog({ referralId, onClose, onChanged, notify }: { referralId: string; onClose: () => void; onChanged: () => void; notify: (message: string) => void }) {
  ...
  return (
    <div className="dialog-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
      <section className="request-dialog operation-case-dialog referral-review-dialog" data-testid="operation-referral-dialog" role="dialog" aria-modal="true" aria-labelledby="referral-review-title">
...
const supportCaseOccurrenceLabel: Record<SupportCase["status"], string> = {
  open: "Ocorrência aberta",
  in_review: "Ocorrência em análise",
    ...
    <>
      ...
      <section className="dashboard-section records-card">
        ...
        <div className="record-list">
          ...
          {filteredBookings.map((booking) => <button key={booking.id} data-testid="booking-record" data-request-code={booking.requestCode} onClick={() => setSelectedBookingId(booking.id)}><span className="record-code">{booking.requestCode}</span><span><strong>{booking.requestTitle}</strong><small>{role === "cliente" ? booking.providerName : booking.customerName} · {dateTime(booking.scheduledFor)}</small></span><span className={`status-pill ${booking.status === "cancelled" ? "warning" : "success"}`}>{bookingStatusLabel[booking.status]}</span><i>→</i></button>)}
          {filteredRequests.map((request) => <button key={request.id} data-testid="request-record" data-request-code={request.publicCode} onClick={() => setSelectedRequest(request)}><span className="record-code">{request.publicCode}</span><span><strong>{request.title}</strong><small>{request.proposalCount > 0 ? `${request.proposalCount} proposta(s)` : request.preferredWindow}</small></span><span className="status-pill success">{request.status === "proposals_received" ? "Propostas" : "Aberto"}</span><i>→</i></button>)}
        </div>
      </section>
...
function BookingDetailDialog({ role, bookingId, onClose, onChanged, notify }: { role: "cliente" | "prestador"; bookingId: string; onClose: () => void; onChanged: () => void; notify: (message: string) => void }) {
  ...
  return (
    <div className="dialog-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
      <section className="request-dialog booking-dialog" data-testid="booking-dialog" role="dialog" aria-modal="true" aria-labelledby="booking-title">
        <button className="dialog-close" ref={closeRef} onClick={onClose} aria-label="Fechar">×</button>
        ...
        {booking && <>
          ...
          <div className="booking-dialog-body">
            ...
            {booking.status === "cancelled" && booking.cancellationReason && <section className="cancellation-summary" data-testid="cancellation-summary"><div><small>MOTIVO DO CANCELAMENTO</small><strong>{cancellationReasonLabel[booking.cancellationReason]}</strong></div><p>{booking.cancellationDetails}</p><footer><span>Solicitado por {booking.cancelledByName}</span><strong>{booking.supportCaseCode} · {booking.supportCaseStatus ? supportCaseOccurrenceLabel[booking.supportCaseStatus] : "Ocorrência registrada"}</strong></footer></section>}
...
function ProposalComparisonDialog({ request, onClose, onChanged, notify }: { request: PersistedRequest; onClose: () => void; onChanged: () => void; notify: (message: string) => void }) {
  ...
  return (
    <div className="dialog-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
      <section className="request-dialog comparison-dialog" data-testid="proposal-comparison-dialog" role="dialog" aria-modal="true" aria-labelledby="comparison-title">
        <button className="dialog-close" ref={closeRef} onClick={onClose} aria-label="Fechar">×</button>
        ...
        <div className="comparison-list">
          ...
          {proposals.map((proposal) => (
            <article key={proposal.id} data-testid="proposal-card" className={`comparison-card ${proposal.status === "accepted" ? "accepted" : proposal.status === "declined" ? "declined" : ""}`}>
