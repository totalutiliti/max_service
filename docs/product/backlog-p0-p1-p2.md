...
- ampliar idempotência para uploads binários e adicionar E2E/a11y em navegador; criação de pedido, envio de proposta, aceite, mensagens de texto, abertura e atualização de atendimentos, agenda, ciclo do serviço e todos os comandos operacionais JSON já possuem chave assinada e garantia transacional, com 23 ações exercitadas por teste concorrente, enquanto migrations, RLS, isolamento da agenda, rate limit, headers HTTP, CORS e limites de corpo também são testados automaticamente no CI;
