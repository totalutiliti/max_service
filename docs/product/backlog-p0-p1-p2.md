...
- integrar o exportador OpenMetrics protegido já materializado a um coletor gerenciado, retenção, traces, alertas e SLOs; liveness, readiness, cockpit operacional, IDs de correlação, logs JSON sem PII, métricas de baixa cardinalidade e smoke tests de autorização já estão materializados;
