...
- adicionar E2E/a11y em navegador; criação de pedido, envio de proposta, aceite, mensagens, abertura e atualização de atendimentos, agenda, ciclo do serviço, todos os comandos operacionais JSON e quatro uploads privados já possuem chave assinada e garantia transacional, com 27 ações exercitadas por teste concorrente; a reconciliação diária preserva arquivos referenciados e recentes e expurga somente órfãos elegíveis, enquanto migrations, RLS, isolamento da agenda, rate limit, headers HTTP, CORS e limites de corpo também são testados automaticamente no CI;
