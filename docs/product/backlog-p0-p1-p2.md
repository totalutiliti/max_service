...
- manter e ampliar o E2E conforme novas jornadas forem adicionadas; Playwright já conduz integralmente pela interface a criação de pedido, envio de proposta, escolha de horário, aceite, mensagens bilaterais, início, conclusão e avaliação; cancelamento, abertura automática de ocorrência, atribuição, resolução operacional e acompanhamento final pelo cliente; indicação de profissional, análise, aprovação para onboarding e retorno do status ao parceiro; e atendimento compartilhado da rede, com mensagem, triagem, SLA, resposta e resolução; acesso, quatro perfis, dezesseis áreas, versão móvel, teclado, foco, revogação de sessão e WCAG 2.2 AA também estão cobertos; todos os comandos operacionais JSON e quatro uploads privados possuem chave assinada e garantia transacional, com 27 ações exercitadas por teste concorrente; reconciliação, migrations, RLS, isolamento da agenda, rate limit, headers HTTP, CORS e limites de corpo também são testados automaticamente no CI;
