...
A central de atendimento da rede usa tabelas próprias para não transformar notas internas de cancelamento em comunicação externa. O parceiro pode criar casos apenas para si, vincular somente indicações da própria rede e inserir mensagens em casos não resolvidos. A Operação lê a fila completa, responde e executa somente as transições `open → in_review → resolved`, sempre com justificativa registrada em evento e auditoria. A identidade nominal da equipe é projetada como “Equipe Max” para o parceiro, evitando ampliar a política de leitura da tabela de usuários.
