...
A Central de Privacidade aceita solicitações apenas para o ator derivado da sessão; `subject_id` enviado pelo navegador não existe no contrato. Titular e Operação possuem policies separadas de leitura e mutação, eventos são append-only e o responsável operacional precisa assumir a solicitação antes de atualizá-la. A função `SECURITY DEFINER` que expõe e-mail retorna somente a identidade do ator atual, recusa a Operação e não amplia a leitura geral da tabela `users`. O pacote JSON usa consultas filtradas pelo ator mesmo quando a policy de uma tabela oferece contexto mais amplo, como oportunidades abertas ao profissional. Mensagens são representadas apenas por contagem e datas, e indicações não carregam nome ou contato do terceiro. O payload completo não é persistido em tabela própria; no piloto sintético, a resposta idempotente fica limitada à retenção de 24 horas já aplicada às mutações. Produção exige confirmação de identidade real, criptografia e retenção aprovadas para o pacote.
...
A criação de sessão demonstrativa, a indicação pública e a validação de cupom passam por rate limit depois da assinatura interna. Assuntos são transformados por HMAC com chave exclusiva do ambiente; o Redis aplica janela móvel atômica e somente contagens por política chegam à Operação. A indisponibilidade do coordenador fecha as superfícies protegidas com `503`. O controle distribuído comprova o contrato de aplicação, mas não substitui rate limit por IP/conta na borda de produção.
